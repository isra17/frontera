...
class TestMessageBusBackend(unittest.TestCase):
    ...
    def test_get_next_requests(self):
        ...
        _, partition_id, offset = mbb._decoder.decode(mbb.spider_log_producer.messages.pop(0))
        ...
        _, partition_id, netlocs = mbb._decoder.decode(mbb.spider_log_producer.messages.pop(0))
        self.assertEqual((partition_id, netlocs), (0, ['www.example.com']))
        _, partition_id, offset = mbb._decoder.decode(mbb.spider_log_producer.messages.pop(0))
        self.assertEqual((partition_id, offset), (0, 0))
